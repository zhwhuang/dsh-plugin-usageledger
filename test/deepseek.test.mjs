...
test('reads the balance with the API key and normalizes every field', async () => {
  const fake = fakeFetch({
    [`${BASE}/user/balance`]: {
      body: {
        is_available: true,
        balance_infos: [{ currency: 'CNY', total_balance: '43.36', granted_balance: '0.00', topped_up_balance: '43.36' }],
      },
    },
  });
  const balance = await fetchBalance({ baseURL: BASE, apiKey: 'sk-secret', timeoutMs: 1000, fetchImpl: fake.fetchImpl });

  assert.equal(balance.isAvailable, true);
  assert.deepEqual(balance.infos, [{ currency: 'CNY', totalBalance: 43.36, grantedBalance: 0, toppedUpBalance: 43.36 }]);
...
test('keeps both currencies and prefers CNY without converting', async () => {
  ...
  const usdOnly = preferredBalance({ infos: [{ currency: 'USD', totalBalance: 1, grantedBalance: 0, toppedUpBalance: 1 }] });
...
test('an unusable amount becomes null instead of zero', async () => {
  const fake = fakeFetch({
    [`${BASE}/user/balance`]: { body: { is_available: false, balance_infos: [{ currency: 'CNY', total_balance: 'n/a', granted_balance: null, topped_up_balance: '' }] } },
  });
  const balance = await fetchBalance({ baseURL: BASE, apiKey: 'k', timeoutMs: 1000, fetchImpl: fake.fetchImpl });
  assert.deepEqual(balance.infos[0], { currency: 'CNY', totalBalance: null, grantedBalance: null, toppedUpBalance: null });
...
test('maps HTTP failures onto stable codes', async () => {
  for (const [status, code] of [[401, 'UNAUTHORIZED'], [402, 'INSUFFICIENT_BALANCE'], [429, 'RATE_LIMITED'], [500, 'HTTP_500']]) {
...
test('a transport failure and a non-JSON body both raise, never return empty data', async () => {
  await assert.rejects(
    () => fetchModels({ baseURL: BASE, apiKey: 'k', timeoutMs: 1000, fetchImpl: async () => { throw Object.assign(new Error('socket hang up'), { name: 'TypeError' }); } }),
    (error) => error.code === 'NETWORK' && /socket hang up/u.test(error.message),
  );
  const fake = fakeFetch({ [`${BASE}/models`]: () => new Response('<html>', { status: 200, headers: { 'content-type': 'text/html' } }) });
  await assert.rejects(
