...
const AMOUNT_LIVE = envelope({
	...
	days: [
		{
			date: '2026-09-18',
			data: [usageBlock('deepseek-flash', { cacheHit: 900000, cacheMiss: 20000, output: 40000, calls: 120 })]
		},
...
const COST_LIVE = envelope([
	{
		...
		days: [
			{
				date: '2026-09-18',
				data: [{ model: 'deepseek-flash', usage: [{ type: 'PROMPT_CACHE_HIT_TOKEN', amount: '1.50' }] }]
			},
			{
				date: '2026-09-19',
				data: [{ model: 'deepseek-v4-pro', usage: [{ type: 'PROMPT_CACHE_MISS_TOKEN', amount: '0.25' }] }]
			}
		]
	}
]);
...
test('folds the live nested console rows into days, models and window totals', () => {
	...
	assert.deepEqual(
		{ day: second.day, tokens: second.tokens, cost: second.cost },
		{ day: '2026-09-19', tokens: 1500, cost: 0.25 }
	);
...
test('keeps reading the legacy flat row shape', () => {
	...
	assert.deepEqual(usage.complete, {
		allTimeTokens: false,
		allTimeCost: false,
		dailyCost: true,
		dailyTokens: true,
...
test('the lifetime roll-up and the monthly window stay separate figures', () => {
	...
	assert.equal(
		usage.allTimeTokens,
		1558998875 + 7989399 + 4532077 + 1000000 + 100000 + 50000,
		'累计 TOKEN comes from the per-model total roll-up'
	);
...
test('tolerates camelCase rows, a bare array, and amount+price fallbacks', () => {
	const usage = normalizeUsage({
		...
		amount: [
			{ utcDate: '2026-09-02T10:00:00Z', modelName: 'deepseek-flash', usageType: 'request_count', value: 4 }
		],
		cost: [],
		month: '2026-09'
	});
	...
	const usage = normalizeUsage({
		...
		cost: {
			days: [
				{
					date: '2026-09-01',
					data: [{ model: 'deepseek-flash', usage: [{ type: 'PROMPT_TOKEN', amount: 'not-a-number' }] }]
				}
			]
		},
		month: '2026-09'
...
test('requests both console endpoints with the token and unwraps the envelope', async () => {
	...
	const fetchImpl = async (url, init) => {
		calls.push({ url, init });
		const body =
			url.includes('/usage/amount') ? AMOUNT_LIVE
			: url.includes('/usage/cost') ? COST_LIVE
			: SUMMARY_LIVE;
		return new Response(JSON.stringify(body), { status: 200, headers: { 'content-type': 'application/json' } });
	};
	const summary = await fetchSummary({ baseURL: BASE, token: 'console-token', timeoutMs: 1000, fetchImpl });
	assert.equal(
		summary.total_costs[0].amount,
		'164.5208066000000000',
		'both envelopes are peeled to the business payload'
	);
...
test('peels a string-encoded biz_data payload (double-encoded JSON)', async () => {
	const fetchImpl = async url => {
		const inner =
			url.includes('/usage/amount') ? AMOUNT_LIVE.data.biz_data
			: url.includes('/usage/cost') ? COST_LIVE.data.biz_data
			: SUMMARY_LIVE.data.biz_data;
		return new Response(
			JSON.stringify({ code: 0, msg: '', data: { biz_code: 0, biz_msg: '', biz_data: JSON.stringify(inner) } }),
			{
				status: 200,
	...
	assert.equal(
		summary.total_costs[0].amount,
		'164.5208066000000000',
		'the JSON-string biz_data is parsed before unwrapping'
	);
...
test('a single failing series endpoint still yields the other half', async () => {
	const fetchImpl = async url => {
		if (url.includes('/usage/cost')) {
			return new Response(JSON.stringify({ code: 50001, msg: 'boom', data: null }), {
				status: 200,
				headers: { 'content-type': 'application/json' }
			});
		}
		return new Response(JSON.stringify(AMOUNT_LIVE), {
			status: 200,
			headers: { 'content-type': 'application/json' }
		});
	};
...
test('a double console failure raises instead of reporting empty data', async () => {
	const fetchImpl = async () =>
		new Response(JSON.stringify({ code: 50001, msg: 'boom', data: null }), {
			status: 200,
			headers: { 'content-type': 'application/json' }
		});
...
test('a console business code becomes a translatable error', async () => {
	const fetchImpl = async () =>
		new Response(JSON.stringify({ code: 40002, msg: 'Missing Token', data: null }), {
			status: 200,
			headers: { 'content-type': 'application/json' }
		});
	await assert.rejects(
...
test('a biz_code business error is surfaced too', async () => {
	const fetchImpl = async () =>
		new Response(
			JSON.stringify({ code: 0, msg: '', data: { biz_code: 40002, biz_msg: 'Missing Token', biz_data: null } }),
...
test('a business code that cannot be parsed is a failure, not a success', async () => {
	// Regression: `num("FAIL")` returned null, and a null code fell through to
	// the HTTP status — a body the console was rejecting was read as data.
	const fetchImpl = async () =>
		new Response(JSON.stringify({ code: 'FAIL', msg: 'boom', data: null }), {
			status: 200,
			headers: { 'content-type': 'application/json' }
		});
	await assert.rejects(
		() => fetchSummary({ baseURL: BASE, token: 't', timeoutMs: 1000, fetchImpl }),
		error => {
			assert.ok(error instanceof PlatformError);
			assert.equal(error.code, 'CODE_UNKNOWN');
			return true;
		}
	);
});

test('a legacy row that spells the model `name` is kept, not dropped', () => {
	// Regression: `name` sat in TYPE_KEYS as a type fallback, so a row like
	// `{ utc_date, name: 'deepseek-flash', amount }` had its model id read as a
	// metric type, which TOKEN_KINDS rejected — the row vanished silently.
	const usage = normalizeUsage({
		amount: envelope({
			total: [{ utc_date: '2026-09-18', name: 'deepseek-flash', type: 'RESPONSE_TOKEN', amount: 40 }]
		}),
		cost: null,
		month: '2026-09'
	});
	assert.equal(usage.models.length, 1);
	assert.equal(usage.models[0].model, 'deepseek-flash', 'the model id is read from `name`');
	assert.equal(usage.models[0].outputTokens, 40);
	assert.equal(usage.days.length, 1, 'the row feeds the daily series too');
});

test('a legacy row that spells the *type* `name` still reads', () => {
	const usage = normalizeUsage({
		amount: envelope({ total: [{ utc_date: '2026-09-18', name: 'PROMPT_TOKEN', amount: 30 }] }),
		cost: null,
		month: '2026-09'
	});
	// The aggregate prompt bucket lands in its own bucket (excluded from sums),
	// but the row itself must not be dropped or misread as a model.
	assert.equal(
		usage.models.some(row => row.model === 'PROMPT_TOKEN'),
		false,
		'a type spelled `name` never becomes a model'
	);
});
...
test('a flat row parked under `total` feeds the daily series and never the lifetime roll-up', () => {
	...
	const amount = {
		total: [{ utc_date: '2026-09-18', model: 'deepseek-flash', type: 'RESPONSE_TOKEN', amount: 100 }]
	};
...
test('a flat row under `total` cannot inflate the roll-up even when its type is a lifetime one', () => {
	...
	const amount = {
		total: [{ utc_date: '2026-09-18', model: 'deepseek-flash', type: 'PROMPT_CACHE_MISS_TOKEN', amount: 4242 }]
	};
	const usage = normalizeUsage({ summary: {}, amount, cost: null, month: '2026-09' });
...
test('the PROMPT_TOKEN aggregate is excluded from the lifetime token sum', () => {
	...
	// prompt twice.
	const amount = {
		total: [fullBlock('deepseek-flash', { prompt: 1000, cacheHit: 900, cacheMiss: 100, output: 50, calls: 3 })]
	};
	const usage = normalizeUsage({ summary: {}, amount, cost: null, month: '2026-09' });
...
test('shape diagnostics go to the injected logger, never to the console', () => {
	...
	assert.ok(
		seen.some(line => line.includes('usage/amount')),
		'the amount shape is reported'
	);
	assert.ok(
		seen.some(line => line.includes('get_user_summary')),
		'the summary shape is reported'
	);
...
test('a call-count cost row does not fabricate a zero-token model', () => {
	...
	const amount = {
		days: [
			{
				date: '2026-09-18',
				data: [{ model: 'deepseek-v4-pro', usage: [{ type: 'RESPONSE_TOKEN', amount: 100 }] }]
			}
		]
	};
	const cost = {
	...
	assert.deepEqual(
		usage.models.map(entry => entry.model),
		['deepseek-v4-pro'],
		'a call-only model is not a model row'
	);
...
test('a cost-only model still appears, because it really did spend', () => {
	...
	assert.deepEqual(
		usage.models.map(entry => [entry.model, entry.cost]),
		[
			['deepseek-v4-pro', 1.25],
			['deepseek-v4-lite', 0.5]
		]
	);
