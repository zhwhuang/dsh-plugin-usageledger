...
function createFetchDouble({ balanceFails = false, consoleFails = false, rows = true } = {}) {
	...
	const fetchImpl = async (url, init) => {
		...
		if (url.includes('/users/get_user_summary')) {
			...
			return json({
				code: 0,
				data: { total_usage: 1399621931, monthly_costs: [{ cost: 156.63, currency: 'CNY' }] }
			});
		}
		...
		if (url.includes('/usage/cost')) {
			return json({
				code: 0,
				data: {
					total: rows ? [{ utc_date: '2026-09-18', model: 'deepseek-flash', cost: 1.5, currency: 'CNY' }] : []
				}
			});
		}
...
function boot({ services = {}, config = {}, credentials } = {}) {
	...
	const creds =
		credentials ?? createFakeCredentials({ DEEPSEEK_API_KEY: API_KEY, DEEPSEEK_PLATFORM_TOKEN: CONSOLE_TOKEN });
...
test('serves balance and models from the open API, and usage from the console', async () => {
	...
	try {
		...
		assert.deepEqual(
			snapshot.api.models.map(row => row.id),
		...
		assert.equal(
			JSON.stringify(snapshot).includes(CONSOLE_TOKEN),
			false,
			'the console token never reaches the snapshot'
		);
		assert.equal(
			double.calls.some(call => call.url.includes('/user/balance')),
			true
		);
		assert.equal(
			double.calls.some(call => call.url.includes('/api/v0/usage/cost')),
			true
		);
	} finally {
...
test('stores the console token in the credential service and never in a file', async () => {
	...
	try {
		const { harness } = boot({ credentials: creds });
		await delay(20);
		const handler = routeOf(harness, '/api/usageledger/console-token').handler;

		const bad = createFakeResponse();
		await handler(
			createFakeRequest({
				method: 'POST',
				url: '/api/usageledger/console-token',
				body: { token: 'sk-not-a-console-token-value' }
			}),
			bad
		);
		...
		await handler(
			createFakeRequest({
				method: 'POST',
				url: '/api/usageledger/console-token',
				body: { token: CONSOLE_TOKEN }
			}),
			good
		);
		assert.equal(good.status, 200);
		assert.equal(
			creds.store.get('DEEPSEEK_PLATFORM_TOKEN'),
			CONSOLE_TOKEN,
			'the token lands in the credential store'
		);
		assert.deepEqual(
			creds.writes.map(entry => entry.ref),
		...
		await handler(createFakeRequest({ method: 'DELETE', url: '/api/usageledger/console-token' }), cleared);
		...
		await handler(createFakeRequest({ method: 'GET', url: '/api/usageledger/console-token' }), wrongMethod);
...
test('accepts a JSON-wrapped userToken copied straight from Local Storage', async () => {
	...
	try {
		...
		const res = createFakeResponse();
		await handler(
			createFakeRequest({ method: 'POST', url: '/api/usageledger/console-token', body: { token: envelope } }),
			res
		);
...
test('a POST without a token field never clears the stored token', async () => {
	// Regression: the route used to pass `null` whenever the body lacked `token`,
	// and `setConsoleToken(null)` *deletes* the stored credential. A malformed
	// save (`{}`) therefore wiped a working token while answering 400.
	const creds = createFakeCredentials({ DEEPSEEK_API_KEY: API_KEY, DEEPSEEK_PLATFORM_TOKEN: CONSOLE_TOKEN });
	const double = createFetchDouble();
	const original = globalThis.fetch;
	globalThis.fetch = double.fetchImpl;
	try {
		...
		await delay(20);
		const handler = routeOf(harness, '/api/usageledger/console-token').handler;
		assert.equal(creds.store.get('DEEPSEEK_PLATFORM_TOKEN'), CONSOLE_TOKEN, 'precondition: a token is stored');

		for (const body of [{}, { foo: 1 }]) {
			const res = createFakeResponse();
			await handler(createFakeRequest({ method: 'POST', url: '/api/usageledger/console-token', body }), res);
			assert.equal(res.status, 400, `a body without "token" (${JSON.stringify(body)}) is rejected`);
			assert.equal(
				creds.store.get('DEEPSEEK_PLATFORM_TOKEN'),
				CONSOLE_TOKEN,
				'the stored token survives a malformed save'
			);
		}

		// Explicit `token: null` (JSON null) remains the documented way to clear via POST.
		const explicit = createFakeResponse();
		await handler(
			createFakeRequest({ method: 'POST', url: '/api/usageledger/console-token', body: { token: null } }),
			explicit
		);
		assert.equal(explicit.status, 200, 'an explicit JSON null clears the credential');
		assert.equal(creds.store.has('DEEPSEEK_PLATFORM_TOKEN'), false, 'an explicit null clears the credential');
		harness.disposeAll();
	} finally {
		globalThis.fetch = original;
	}
...
test('the hook returns the stream synchronously, as cordis requires', async () => {
	...
	const returned = run({ provider: 'deepseek-official', model: 'deepseek-v4-pro' }, () =>
...
test('a hook that returns a promise is rejected rather than silencing the stream', () => {
	...
	assert.throws(
		() =>
			run({ provider: 'deepseek-official', model: 'deepseek-v4-pro' }, async () =>
				streamOf([{ type: 'text', text: 'hi' }])
			),
		/returned a Promise/,
...
test('follows the in-flight model for the seat without counting anything', async () => {
	...
	const chunks = await collect(
		await run({ provider: 'deepseek-official', model: 'deepseek-v4-pro' }, () =>
			streamOf([
				{ type: 'text', text: 'hi' },
				{ type: 'usage', usage: { inputTokens: 5, outputTokens: 6, totalTokens: 11 } }
			])
		)
	);
	...
	await collect(
		await run({ provider: 'someone-else', model: 'other' }, () => streamOf([{ type: 'text', text: 'x' }]))
	);
...
test('the plugin writes no file of its own', async () => {
	...
	assert.equal(
		entries.some(entry => entry.name.endsWith('-state.json')),
		false,
		'no state document in the package'
	);
	const lib = await readdir(new URL('../lib/', import.meta.url));
	assert.deepEqual(
		lib.filter(name => /state|pricing|balance/iu.test(name)),
		[],
		'the accumulating modules are gone'
	);
	harness.disposeAll();
});
...
test('routes are registered on the web server and reject other methods', async () => {
	...
	void paths;
	for (const path of [
		'/api/usageledger/snapshot',
		'/api/usageledger/events',
		'/api/usageledger/refresh',
		'/api/usageledger/console-token'
	]) {
		assert.ok(routeOf(harness, path) !== undefined, `missing route ${path}`);
	}
	const snapshotHandler = routeOf(harness, '/api/usageledger/snapshot').handler;
	const res = createFakeResponse();
	await snapshotHandler(createFakeRequest({ method: 'POST', url: '/api/usageledger/snapshot' }), res);
...
test('a credential service that arrives after apply is still used', async () => {
	...
	try {
		...
		assert.equal(
			before.snapshot.api.error.code,
			'NO_API_KEY',
			'without the service the read falls back to the environment'
		);
		...
		services.credentials = createFakeCredentials({
			DEEPSEEK_API_KEY: API_KEY,
			DEEPSEEK_PLATFORM_TOKEN: CONSOLE_TOKEN
		});
		const refresh = routeOf(harness, '/api/usageledger/refresh').handler;
		const res = createFakeResponse();
		await refresh(createFakeRequest({ method: 'POST', url: '/api/usageledger/refresh' }), res);
