...
export interface HostContext {
  ...
  on?(event: string, listener: (...args: never[]) => unknown, options?: { global?: boolean; prepend?: boolean }): (() => boolean) | undefined;
...
export interface Snapshot {
  revision: number;
  generatedAt: number;
  api: { baseURL: string; updatedAt: number | null; error: { code: string; message: string } | null; balance: object | null; preferred: BalanceInfo | null; models: ModelInfo[] };
  console: { baseURL: string; updatedAt: number | null; month: string; error: { code: string; message: string } | null; usage: Usage | null };
  credentials: Record<'apiKey' | 'consoleToken', { configured: boolean; source: string | null; writable: boolean; ref: string }>;
