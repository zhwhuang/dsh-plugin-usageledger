/**
 * Browser-facing transport for APICost.
 *
 * Four exact routes are registered on the harness web server:
 *
 * ```text
 * GET  /api/apicost/snapshot   one JSON snapshot
 * GET  /api/apicost/events     Server-Sent Events feed (snapshot per change)
 * POST /api/apicost/refresh    force a balance probe, then answer a snapshot
 * POST /api/apicost/reset      clear counters, then answer a snapshot
 * ```
 *
 ...
 *
 * @module dsh-plugin-apicost/routes
 */

/** Path prefix shared by every route. */
export const ROUTE_PREFIX = '/api/apicost';
...
export function createSseHub({ buildPayload, maxClients, heartbeatMs, logger }) {
  ...
  function writeTo(client) {
    try {
      ...
      logger?.warn?.(`apicost: SSE write failed: ${String(error?.message ?? error)}`);
      drop(client);
    }
  }
  ...
  return {
    handleEventStream(req, res) {
      if (clients.size >= maxClients) {
        writeJson(res, 503, { ok: false, error: { code: 'TOO_MANY_CLIENTS', message: `at most ${maxClients} event streams are supported` } });
        return;
      }
      res.writeHead(200, {
 ...
 * @param {(options?: object) => Promise<object>} params.refreshBalance - balance refresh.
 * @param {(scope: object) => boolean} params.resetState - counter reset.
  ...
  async function guarded(req, res, run) {
    ...
    if (rejection !== undefined) {
      writeJson(res, rejection, {
        ok: false,
        error: { code: rejection === 503 ? 'AUTHENTICATION_UNAVAILABLE' : rejection === 401 ? 'UNAUTHORIZED' : 'FORBIDDEN', message: 'the request was rejected by the harness connection gate' },
      });
      return;
    }
    try {
      ...
      logger?.warn?.(`apicost: route failed: ${String(error?.message ?? error)}`);
      if (!res.headersSent) writeJson(res, 500, { ok: false, error: { code: 'INTERNAL', message: String(error?.message ?? error) } });
      else res.end();
    }
  }
    ...
    async consoleToken(req, res) {
      await guarded(req, res, async () => {
        ...
        const body = await readJsonBody(req).catch((error) => {
          writeJson(res, error?.code === 'TOO_LARGE' ? 413 : 400, { ok: false, error: { code: 'INVALID_BODY', message: String(error?.message ?? error) } });
          return null;
        });
        if (body === null) return undefined;
        const result = await setConsoleToken(typeof body.token === 'string' ? body.token : null);
        if (result.configured !== true) {
          return writeJson(res, 400, { ok: false, error: { code: result.code ?? 'INVALID_TOKEN', message: result.message ?? 'the console token was rejected' } });
        }
        writeJson(res, 200, { ok: true, snapshot: buildSnapshot(), result });
      });
    },
