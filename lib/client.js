	...
	factory: require => {
		...
		const css = [
		   ...
		   account cards so the two bases are never read as one figure */
			...
			'.apx-platforms{margin-top:14px;display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:10px}',
			'.apx-pcard{background:var(--surface-2);border:1px solid var(--rule);border-radius:var(--r-md);padding:12px 14px;display:flex;flex-direction:column;gap:8px}',
			'.apx-pcard-head{display:flex;align-items:center;gap:8px}',
			'.apx-pcard-name{font-size:13px;font-weight:600;color:var(--ink)}',
			'.apx-pcard-live{margin-left:auto;font-family:var(--mono);font-size:9px;letter-spacing:.08em;text-transform:uppercase;color:var(--accent)}',
			'.apx-pcard-balance{display:flex;flex-direction:column;gap:2px}',
			'.apx-pcard-money{font-family:var(--mono);font-size:16px;font-weight:600;font-variant-numeric:tabular-nums;color:var(--ink)}',
			'.apx-pcard-money small{margin-left:3px;font-size:9px;color:var(--ink-3)}',
		...
		if (typeof document !== 'undefined') {
			const tagId = 'dsh-plugin-usageledger/ui.css';
			if (document.querySelector('style[data-plugin-css=' + JSON.stringify(tagId) + ']') === null) {
				const tag = document.createElement('style');
				tag.dataset.plugin = 'dsh-plugin-usageledger';
		...
		const DICT = {
			zh: {
				...
				consoleOff: '平台控制台未连接',
				consoleWhy:
					'累计使用金额、累计 TOKEN 与每日趋势来自平台控制台的用量接口（官方 API 只有余额与模型列表），需要一次性授权控制台令牌——不是 API Key。',
				consoleAuto: '一键读取',
				...
				consoleManual: '手动粘贴',
				consoleHow:
					'若自动读取不可用，在已登录 platform.deepseek.com 的浏览器里打开开发者工具 → Application → Local Storage → userToken，复制它的值粘贴到这里。',
				...
				timeoutHint: '网络超时，可稍后重试，或在 profile 配置里调大 timeoutMs',
				platformsTitle: '平台',
				platformNoBalance: '该平台暂未开放余额接口，此处仅显示会话用量',
				platformMeteringOnly: '仅会话用量（未识别平台 API）',
				platformAddHint: '在设置 → 模型页添加提供方后，这里会自动出现它的账单卡',
				keyRegionMismatch: 'API Key 与端点地域不匹配（跨地域调用会被拒）',
				retry: '重新读取',
				refreshing: '读取中…',
			...
			en: {
				...
				consoleWhy:
					'All-time spend, all-time tokens and the daily trend come from the console usage endpoints — the open API only serves balance and the model list — so a one-off console token is needed (not an API key).',
				consoleHow:
					'Open devtools on a signed-in platform.deepseek.com → Application → Local Storage → userToken and copy its value.',
				consoleAuto: 'Read it for me',
				consoleAutoHint:
					'Signed in to platform.deepseek.com in this browser? Read the token automatically — no copy-paste.',
				consoleSameOrigin:
					'This page is on platform.deepseek.com, so the signed-in token can be read directly.',
				consoleManual: 'Paste manually',
				consoleClear: 'Disconnect',
				consoleSaved:
					'The token is written to the dsh credential store only: no plugin file, no log line, never sent to the browser.',
				...
				timeoutHint: 'The request timed out; retry later or raise timeoutMs in the profile config',
				platformsTitle: 'Platforms',
				platformNoBalance: 'This platform publishes no balance API; only session usage is shown here',
				platformMeteringOnly: 'Session usage only (no platform API recognized)',
				platformAddHint: 'Add a provider on the Models settings page and its card appears here automatically',
				keyRegionMismatch: 'API key and endpoint region mismatch (cross-region calls are rejected)',
				retry: 'Read again',
				refreshing: 'Reading…',
				...
				sessionScope:
					'Counted by the harness for this session alone — a different basis from the platform bill, and never added to it.',
		...
		function t(key, vars) {
			...
			return raw.replace(/\{(\w+)\}/gu, (match, name) =>
		...
		/**
		 * Human text for one snapshot error.
		 *
		 * Recognized codes are translated into actionable copy — a raw
		 * `TIMEOUT · request timed out after 8000ms` is a diagnosis without a
		 * remedy, so the panel says what to do instead. Unrecognized codes fall
		 * through as `CODE · message`, so a new backend code stays visible
		 * rather than being swallowed by an incomplete dictionary.
		 *
		 * @param {{ code: string, message: string } | null | undefined} error - the snapshot error.
		 * @returns {string | null} the text, or null when there is no error.
		 */
		function errorText(error) {
			if (error === null || error === undefined) return null;
			if (error.code === 'NO_API_KEY') return t('noApiKey');
			if (error.code === 'EXPIRED' || error.code === 'NO_TOKEN') return t('expired');
			if (error.code === 'TIMEOUT') return error.code + ' · ' + t('timeoutHint');
			if (error.code === 'KEY_REGION_MISMATCH') return error.code + ' · ' + t('keyRegionMismatch');
		...
		const SNAPSHOT_URL = '/api/usageledger/snapshot';
		const EVENTS_URL = '/api/usageledger/events';
		const REFRESH_URL = '/api/usageledger/refresh';
		const TOKEN_URL = '/api/usageledger/console-token';
		/**
		...
		/**
		 * Browser-side per-request floor. The host's real timeout arrives with the
		 * snapshot (`hostTimeoutMs`); until then this floor keeps a slow first load
		 * from reporting failure while the host is still legitimately working.
		 */
		const REQUEST_TIMEOUT_FLOOR_MS = 10000;
		let requestTimeoutMs = REQUEST_TIMEOUT_FLOOR_MS;
		/**
		...
		function createCostStore(options = {}) {
			...
			function pollIntervalFor(snapshot) {
				...
				const usageReady =
					(snapshot && snapshot.console && snapshot.console.usage !== null) || !tokenConfigured;
			...
			async function fetchSnapshot() {
				try {
					...
					if (!ok || payload === null || payload.ok !== true)
						throw new Error((payload && payload.error && payload.error.message) || 'snapshot failed');
					const hostTimeout = Number(payload.snapshot && payload.snapshot.hostTimeoutMs);
					if (Number.isFinite(hostTimeout) && hostTimeout > requestTimeoutMs) requestTimeoutMs = hostTimeout;
			...
			return {
				...
				async refresh() {
					...
					try {
						...
						if (!ok || payload === null || payload.ok !== true)
							throw new Error((payload && payload.error && payload.error.message) || 'refresh failed');
				...
				async saveConsoleToken(token) {
					...
					try {
						...
						if (!ok || payload === null || payload.ok !== true) {
							...
							return {
								ok: false,
								code: (failure && failure.code) || 'STORE_FAILED',
		...
		/**
		 * One platform's billing card, generated from what the harness reaches.
		 *
		 * There is no fixed card set: the detail window renders one of these per
		 * entry in the snapshot's `platforms` list, so a provider added on the
		 * models page grows a card here and a removed one loses it. The three
		 * areas (balance / models / metering note) follow the platform's
		 * declared capabilities — a metering-only gateway shows why it has no
		 * balance instead of an error.
		 *
		 * @param {object} props - `platform` (one snapshot `platforms` entry) and
		 *   `currency` fallback for the balance card.
		 */
		function PlatformCard({ platform, currency }) {
			const facts = platform ?? {};
			const caps = facts.capabilities ?? {};
			const balance = caps.balance === true ? facts.balance : null;
			const preferred =
				balance === null ? null
				: balance.infos && balance.infos.length > 0 ?
					(balance.infos.find(info => info.currency === 'CNY') ?? balance.infos[0])
				:	null;
			const models = Array.isArray(facts.models) ? facts.models : [];
			const meteringOnly = caps.balance !== true && caps.models !== true;
			...
			return h(
				...
				h(
					'div',
					{ className: 'apx-pcard-head' },
					h('span', { className: 'apx-pcard-name' }, String(facts.label ?? facts.id ?? '')),
					facts.observed === true ? h('span', { className: 'apx-pcard-live' }, t('liveNow')) : null
				),
				meteringOnly ?
					h('p', { className: 'apx-scope-note' }, t('platformMeteringOnly'))
				:	h(
						React.Fragment,
						null,
						caps.balance === true ?
							h(
								'div',
								{ className: 'apx-pcard-balance' },
								h('span', { className: 'apx-card-k' }, t('balance')),
								h(
									'span',
									{ className: 'apx-pcard-money' },
									preferred === null ? '—' : (
										money(preferred.currency ?? currency, preferred.totalBalance)
									),
									preferred !== null && preferred.currency ?
										h('small', null, preferred.currency)
									:	null
								)
							)
						:	h('p', { className: 'apx-scope-note' }, t('platformNoBalance')),
						models.length > 0 ?
							h(
								'div',
								...
								models
									.slice(0, 12)
									.map(entry => h('span', { key: entry.id, className: 'apx-chip' }, entry.id))
							)
						:	null
					),
				facts.error !== null && facts.error !== undefined ?
					h('p', { className: 'apx-warn' }, errorText(facts.error))
				:	null
			);
		}

		/**
		 * The platform section: one generated card per discovered platform, with
		 * the empty-state hint pointing at the models page rather than a local
		 * credential form.
		 *
		 * @param {object} props - `platforms` (snapshot list) and `currency`.
		 */
		function PlatformSection({ platforms, currency }) {
			const list = Array.isArray(platforms) ? platforms : [];
			if (list.length === 0) return h('p', { className: 'apx-scope-note' }, t('platformAddHint'));
			return h(
				'div',
				{ className: 'apx-platforms' },
				list.map(entry => h(PlatformCard, { key: entry.id, platform: entry, currency }))
			);
		}
		...
		function DailyStrip({ month, days, metric, format }) {
			...
			if (!Number.isFinite(year) || !Number.isFinite(monthNumber))
				return h('div', { className: 'apx-empty' }, t('empty'));
				...
				METRICS.map(entry =>
					h(
						'button',
						{
							key: entry.id,
							type: 'button',
							className: metric === entry.id ? 'is-on' : undefined,
			...
			if (props.wide === false) {
				const railMoney =
					preferred === null ?
						sessionUsage === null ?
							'--'
						:	compact(sessionUsage.totalTokens)
					:	money(preferred.currency, preferred.totalBalance, 1);
				return h(
					'div',
				...
				h(
					...
					h(
						...
						h(
							...
							sessionUsage === null ? null : (
								h(
									'span',
									{ className: 'apx-seat-session' },
									compact(sessionUsage.totalTokens) + ' ' + t('metricTokens')
								)
							),
							h('span', { className: dotClass(status) })
						)
		...
		function ConnectionPill({ tone, label, detail }) {
			return h(
				'span',
				{
					className: 'apx-pill apx-pill--' + tone,
					title: detail === null || detail === undefined ? label : String(detail)
				},
				h('i', null),
				h('b', null, label),
		...
		function SessionScope({ session }) {
			...
			return h(
				...
				h(
					...
					// is only one.
					Number(facts.sessions) > 1 ?
						h('span', { className: 'apx-scope-id' }, '· ' + t('sessionOf', { n: facts.sessions }))
					:	null
				),
				usage === null ?
				...
				:	h(
						...
						h(
							...
							usage.cacheWriteTokens === 0 ? null : h('dt', null, t('sessionCacheWrite')),
							usage.cacheWriteTokens === 0 ? null : h('dd', null, count(usage.cacheWriteTokens)),
					...
					h(
						...
						h(
							...
							h(
								'dd',
								null,
								compact(context.surfaceTokens) + (context.approximated ? ' ' + t('sessionApprox') : '')
							)
						)
					)
				),
		...
		function ConsoleCard({ consoleFacts, credentials, store, state }) {
			...
			function submit(candidate) {
				void store.saveConsoleToken(candidate).then(result => {
					...
					if (code === 'CREDENTIALS_UNAVAILABLE') setNotice(t('consoleNoStore'));
						...
						setNotice(t('consoleStoreFailed') + (detail === '' ? '' : ' · ' + detail));
			...
			if (configured) {
				...
				return h(
					...
					h(
						'p',
						{ className: 'apx-warn' },
						error.code === 'EXPIRED' || error.code === 'NO_TOKEN' ? t('expired') : errorText(error)
					)
				);
			}
			...
			return h(
					...
					h(
						...
						h(
							'button',
							{
								...
								disabled: state.saving === true || !writable,
								onClick: autoRead
							},
				...
				error === null || error.code === 'NO_CONSOLE_TOKEN' ?
					null
				:	h('p', { className: 'apx-warn' }, errorText(error))
			);
		}

		function UsageLedgerOverlay(props) {
			const state = useCost(props.cost);
			...
			const apiKeyOn =
				...
				credentials.apiKey.configured === true;
			...
			const consoleOn =
				credentials !== null &&
				credentials.consoleToken !== null &&
				credentials.consoleToken !== undefined &&
				credentials.consoleToken.configured === true;
			const consoleBroken = consoleError !== null && usage === null;

			return h(
				'div',
				{
					className: 'apx apx-ovl',
					onClick: close,
				...
				h(
					...
					h(
						...
						h(
							...
							h(
								...
								h(ConnectionPill, {
									...
									detail:
										consoleOn ?
											consoleBroken ? consoleError.code
											:	t('connOn')
										:	t('connOff')
								}),
								data && data.live && data.live.streaming === true ?
									h(ConnectionPill, { tone: 'live', label: t('liveNow') })
								:	null,
								h(
									'button',
									{
								...
								consoleOn ?
									h(
										'button',
										{
											...
											onClick: () => props.cost.clearConsoleToken()
										},
										t('consoleClear')
									)
								:	null
						...
						api.error !== null && api.error !== undefined ?
							h('p', { className: 'apx-warn' }, errorText(api.error))
						:	null,
						...
						h(
							...
							h(
								...
								h(
									'a',
									{
										className: 'apx-btn apx-btn--go apx-card-cta',
						...
						h(
							...
							h(
								...
								h(
									'span',
									{ className: 'apx-trend-s' },
									String(consoleFacts.month ?? '—') +
										' · ' +
										t('updatedAt') +
										' ' +
										clock(consoleFacts.updatedAt)
								)
							),
							usage === null ?
							...
							:	h(
									...
									h(
										...
										h(Stat, {
											...
											value:
												monthWindow === null ? '—' : (
													money(usage.currency ?? currency, monthWindow.cost)
												),
											unit: monthWindow === null ? undefined : (usage.currency ?? currency)
										}),
										h(Stat, {
											label: t('monthTokens'),
											value: monthWindow === null ? '—' : compact(monthWindow.tokens)
										}),
										h(Stat, {
											label: t('monthCalls'),
											value: monthWindow === null ? '—' : count(monthWindow.calls)
										})
									)
								)
						),
						...
						h(
							...
							h(
								...
								api.updatedAt === null || api.updatedAt === undefined ?
									null
								:	h('span', { className: 'apx-trend-s' }, t('updatedAt') + ' ' + clock(api.updatedAt)),
								h(
									'button',
									{
										type: 'button',
										...
										'aria-label':
											state.modelsPending === true ? t('modelsRefreshing') : t('modelsRefresh'),
								...
								:	(api.models ?? []).map(entry =>
										h(
											'span',
											{
												key: entry.id,
												className:
													'apx-chip' + (model.label === entry.id ? ' is-on' : undefined)
											},
											entry.id
										)
									)
						...
						h(PlatformSection, { platforms: data && data.platforms, currency }),
		...
		function apply(ctx) {
			...
			ctx.slots.inject('shell.overlay', () => ctx.slots.register(overlay, UsageLedgerOverlay));
		}

		exports.apply = apply;
		exports.inject = inject;
		exports.SidebarCostWidget = SidebarCostWidget;
		exports.UsageLedgerOverlay = UsageLedgerOverlay;
