 ...
 *
 * The signal-abort failure is not a single shape. Node raises `AbortError`
 * (an `Error` subclass), but a DOM runtime may surface the same abort as a
 * `DOMException` with `name === 'TimeoutError'`, and fetches aborted through
 * other paths can carry the signal's `abort.reason` instead — usually a plain
 * `Error` whose message spells out the abort. Matching on `name === 'AbortError'`
 * alone misfiles all of those as `NETWORK`, and the panel then shows a raw
 * socket string where it should show a translated TIMEOUT pill.
 *
...
async function requestJson({ url, apiKey, timeoutMs, fetchImpl = globalThis.fetch }) {
	...
	if (payload === null || typeof payload !== 'object')
		throw new DeepSeekError('MALFORMED', 'response body was not JSON');
	return payload;
}
