 ...
 *
 * @module dsh-plugin-apicost/platform
 */
...
/** Field names the console has used for one row's metric type. */
...
function unwrap(payload) {
  ...
  for (let pass = 0; pass < 5; pass += 1) {
    ...
    const dataKey = record.data !== undefined ? 'data' : record.biz_data !== undefined ? 'biz_data' : undefined;
...
async function consoleJson({ url, token, timeoutMs, fetchImpl = globalThis.fetch }) {
  ...
  try {
    ...
    throw new PlatformError(aborted ? 'TIMEOUT' : 'NETWORK', aborted ? `request timed out after ${timeoutMs}ms` : String(error?.message ?? error));
  } finally {
  ...
  if (payload === null || typeof payload !== 'object') {
    throw new PlatformError(response.ok ? 'MALFORMED' : `HTTP_${response.status}`, 'response body was not JSON', response.status);
  }
  ...
  if (code !== null && code !== 0 && code !== 200) {
    const message =
      str(inner.biz_msg) ?? str(record.biz_msg) ?? str(record.msg) ?? str(record.message) ?? str(inner.msg) ?? `console code ${code}`;
    const mapped = code === 40002 ? 'NO_TOKEN' : response.status === 401 ? 'EXPIRED' : `CODE_${code}`;
    throw new PlatformError(mapped, message, response.status);
  }
  if (!response.ok) throw new PlatformError(response.status === 401 ? 'EXPIRED' : `HTTP_${response.status}`, `HTTP ${response.status}`, response.status);
...
export async function fetchUsage({ baseURL, token, year, month, timeoutMs, fetchImpl }) {
  ...
  const failed = amount.status === 'rejected' ? 'amount' : cost.status === 'rejected' ? 'cost' : null;
  return {
    ...
    partialMessage: failed === null ? null : String((failed === 'amount' ? amount.reason : cost.reason)?.message ?? ''),
  };
}

/** One empty daily bucket. */
function emptyDay(day) {
  return { day, calls: 0, promptTokens: 0, cacheHitTokens: 0, cacheMissTokens: 0, outputTokens: 0, tokens: 0, cost: 0, costKnown: false };
}

/** One empty per-model bucket. */
function emptyModel(model) {
  return { model, calls: 0, promptTokens: 0, cacheHitTokens: 0, cacheMissTokens: 0, outputTokens: 0, tokens: 0, cost: null };
}
...
function usageEntries(payload, isCost = false, section = 'days') {
  ...
  const pushUsage = (day, model, usage, amountKeys = AMOUNT_KEYS) => {
    ...
    for (const item of usage) {
      ...
      const type = str(pick(obj, TYPE_KEYS)) ?? '';
  ...
  const days = Array.isArray(root.days) ? root.days : looksLikeDays(root.data) ? root.data : null;
  const dayList = Array.isArray(days) ? days : null;
  const flatRows = Array.isArray(root) ? root : null;
  const totalList = Array.isArray(root.total) ? root.total : Array.isArray(root.totals) ? root.totals : null;
  ...
  if (section === 'total') {
    ...
    for (const modelEntry of totalList) {
      ...
      if (Array.isArray(usage)) pushUsage(null, str(pick(modelObj, MODEL_KEYS)), usage, amountKeys);
    }
    return entries;
  }
    ...
    for (const dayEntry of dayList) {
      ...
      if (Array.isArray(data)) {
        for (const modelEntry of data) {
          const modelObj = asObject(modelEntry);
          if (modelObj === null) continue;
          pushUsage(day, str(pick(modelObj, MODEL_KEYS)), modelObj.usage ?? modelObj.items ?? modelObj.rows, amountKeys);
        }
      } else {
        pushUsage(day, str(pick(dayObj, MODEL_KEYS)), dayObj.usage, amountKeys);
      }
    }
  ...
  if (flatRows !== null) {
    for (const row of flatRows) {
      ...
      pushUsage(dayOf(pick(line, DATE_KEYS)), str(pick(line, MODEL_KEYS)), [line], amountKeys);
    }
    return entries;
  }
  ...
  if (totalList !== null) {
    for (const modelEntry of totalList) {
      ...
      if (modelObj === null) continue;
      const usage = modelObj.usage ?? modelObj.items ?? modelObj.rows;
      if (!Array.isArray(usage)) pushUsage(dayOf(pick(modelObj, DATE_KEYS)), str(pick(modelObj, MODEL_KEYS)), [modelObj], amountKeys);
    }
  }
  return entries;
}
...
function walletBalance(entry) {
  ...
  return num(pick(obj, ['balance', 'amount', 'total_balance', 'totalBalance', 'topped_up_balance', 'toppedUpBalance']));
}
...
export function normalizeUsage({ summary, amount, cost, month, logger }) {
  ...
  const remainingTokens =
    num(summaryPick(summaryRecord, ['token_estimation', 'total_available_token_estimation', 'totalAvailableTokenEstimation', 'available_tokens', 'remaining_tokens'])) ??
    (Array.isArray(normalWallets) ? num(pick(asObject(normalWallets[0]) ?? {}, ['token_estimation', 'tokenEstimation'])) : null);
  ...
  const costList = Array.isArray(totalCosts) && totalCosts.length > 0 ? totalCosts : Array.isArray(monthlyCostList) ? monthlyCostList : [];
  ...
  const explicitAllTimeCost = num(
    summaryPick(summaryRecord, ['total_cost', 'totalCost', 'all_time_cost', 'allTimeCost', 'total_spend', 'totalSpend', 'lifetime_cost', 'lifetimeCost']),
  );
  const spendCurrency = Object.keys(spendByCurrency).includes('CNY') ? 'CNY' : (Object.keys(spendByCurrency)[0] ?? null);
  const allTimeCost = explicitAllTimeCost ?? (spendCurrency === null ? null : Number(spendByCurrency[spendCurrency].toFixed(6)));
  ...
  const dayList = [...days.values()]
    .map((entry) => ({ ...entry, tokens: tokenSum(entry), cost: entry.costKnown ? Number(entry.cost.toFixed(6)) : null }))
    .sort((left, right) => left.day.localeCompare(right.day));
  const modelList = [...models.values()]
    .map((entry) => ({ ...entry, tokens: tokenSum(entry), cost: entry.cost === null ? null : Number(entry.cost.toFixed(6)) }))
  ...
  const allTimeTokens =
    num(summaryPick(summaryRecord, ['total_usage', 'totalUsage', 'total_tokens', 'totalTokens', 'all_time_tokens', 'allTimeTokens', 'lifetime_tokens', 'tokens_used'])) ??
    lifetimeTokens;
  ...
  const monthlyTokens = num(summaryPick(summaryRecord, ['monthly_usage', 'monthlyUsage', 'monthly_tokens', 'current_month_tokens'])) ?? (windowTokens > 0 ? windowTokens : null);
  ...
  if (diagnose !== null) {
    if (amountEntries.length === 0 && amountValue !== null && amountValue !== undefined) {
      diagnose(`apicost: usage/amount produced no readable entries; payload shape: ${shapeSignature(asObject(amountValue) ?? amountValue)}`);
    }
    if (costEntries.length === 0 && costValue !== null && costValue !== undefined) {
      diagnose(`apicost: usage/cost produced no readable entries; payload shape: ${shapeSignature(asObject(costValue) ?? costValue)}`);
    }
    if (Object.keys(summaryRecord).length > 0 && allTimeCost === null && remainingTokens === null) {
      diagnose(`apicost: get_user_summary produced no usable totals; shape: ${shapeSignature(summaryRecord)}`);
    }
  }
  ...
  return {
    ...
    wallets: {
      ...
      currency: spendCurrency ?? (Array.isArray(normalWallets) ? str(pick(asObject(normalWallets[0]) ?? {}, CURRENCY_KEYS)) : null),
    },
